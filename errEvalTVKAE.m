function [errTVKAE]=errEvalTVKAE(stDevIn,IMUIndex,speedIndex,beta,verbose)
...
if nargin<2 || isempty(IMUIndex),   IMUIndex=1;   end % Index of the IMU type
...
switch IMUIndex
    case 1, IMUType='XS'; % IMUs de XSens
    case 2, IMUType='AP'; % IMUs de APDM
    case 3, IMUType='SH'; % IMUs de Shimmer
...
for IMUIndex=1:2
  ...
  S=eval(sprintf('%s%d',IMUType,IMUIndex));
  ...
  % Angle between gravitational and magnetic fields (assumed to be constant)
  alphaCos=Zeb'*Heb;
  ...
  for k=2:N
      [X(:,k),P]=TVKAE(X(:,k-1),Z(:,k),P,Dt,Q,R,alphaCos,alphaSin,g,beta);
  end
